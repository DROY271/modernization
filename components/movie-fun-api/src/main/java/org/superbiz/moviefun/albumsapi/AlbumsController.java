...
public class AlbumsController {
    ...
    private final CoverCatalog covers;

    public AlbumsController(AlbumsClient albumsClient, CoverCatalog covers) {
    ...
    public String uploadCover(@PathVariable Long albumId, @RequestParam("file") MultipartFile uploadedFile) {
        ...
        if (uploadedFile.getSize() > 0) {
            try {
                covers.uploadCover(albumId, uploadedFile.getInputStream(), uploadedFile.getContentType());
            } catch (IOException e) {
    ...
    public HttpEntity<byte[]> getCover(@PathVariable long albumId) throws IOException, URISyntaxException {
        ...
        Blob coverBlob = covers.getCover(albumId);
